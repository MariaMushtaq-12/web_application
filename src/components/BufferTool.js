// src/components/BufferTool.js
import React, { useState, useEffect } from 'react';

const BufferTool = ({ setBufferParams, clickedCoordinates, onClose }) => {
  const [bufferCenter, setBufferCenter] = useState(null);
  const [bufferRadius, setBufferRadius] = useState(1000);

  useEffect(() => {
    if (clickedCoordinates) {
      setBufferCenter(clickedCoordinates);
    }
  }, [clickedCoordinates]);

  const handleSubmit = (e) => {
    e.preventDefault();
    if (bufferCenter && bufferRadius) {
      setBufferParams({ center: bufferCenter, radius: bufferRadius });
    } else {
      alert('Please select a center point and specify a radius for the buffer.');
    }
  };

  return (
    <div className="buffer-tool">
      <section className="bg-gray-50 dark:bg-gray-900">
        <div className="w-full bg-gray-600 rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            <h1 className="text-xl font-bold leading-tight tracking-tight text-white md:text-2xl dark:text-white">
              Create Buffer
            </h1>
            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 gap-4">
                <div>
                  <label htmlFor="bufferRadius" className="block mb-2 text-sm font-medium text-white dark:text-white">
                    Buffer Radius (meters):
                  </label>
                  <input
                    type="number"
                    id="bufferRadius"
                    value={bufferRadius}
                    onChange={(e) => setBufferRadius(Number(e.target.value))}
                    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Enter buffer radius"
                    required
                  />
                </div>
              </div>
              <button type="submit" className="w-full text-white bg-black hover:bg-green-500 hover:text-gray-900 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                Create Buffer
              </button>
              <button onClick={onClose} className="w-full text-white bg-red-500 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mt-4">
                Close
              </button>
            </form>
          </div>
        </div>
      </section>
    </div>
  );
};

export default BufferTool;
